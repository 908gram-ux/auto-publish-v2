...
class ImageJudge {
    ...
    public function judge(array $candidates, array $ctx) {
        ...
        $user .= "## 평가 기준 (엄격하게! 애매하면 낮은 점수)\n";
        $user .= "- score (0~10): 글의 주제·분위기와 얼마나 맞는가. 주제와 무관(지도, 광고, 메뉴판, 캡처화면, 로고, 배너, 프로필사진, 다른 제품/인물/업종)이면 0~2.\n";
        $user .= "- ⛔ 글이 한국의 특정 지역·매장·서비스에 대한 것인데, 이미지 속 인물·배경·간판이 명백히 외국(외국인, 외국 화폐, 해외 매장 등)이면 0~3.\n";
        $user .= "- ⛔ 글 주제가 게임·앱·온라인 서비스 등 가상 콘텐츠인데, 이미지가 무관한 현실 장면(예: 게임 글에 실제 수술실·실제 병원 사진)이면 0~2.\n";
        $user .= "- ⛔ 주제의 일부 단어만 겹치는 이미지(예: '포켓몬 카드 시세' 글에 신용카드 사진, '카드샵' 글에 포커 카드 사진)는 0~3.\n";
        $user .= "- thumb_ok (true/false): 대표 이미지로 써도 되는가. 가로형이고, 글자/워터마크/자막/말풍선이 거의 없고, 글 제목을 오해 없이 대표하는 깨끗한 사진일 때만 true. 세로형, 캡처화면, 표/그래프, 글자 가득한 이미지, 주제를 오해하게 만드는 이미지는 false.\n";
        $user .= "- has_text (true/false): 이미지 안에 글자/자막/워터마크/로고가 눈에 띄게 있는가.\n";
        $user .= "- reason: 10~20자 한국어 한 줄 이유.\n";
        $user .= "- caption: 이 이미지 **실제 내용**을 설명하는 15~30자 한국어 캡션. 반드시 이미지에 보이는 것만 그대로 묘사(예: 순위표 캡처면 '2026 K리그1 순위표', 경기 사진이면 '경기 중인 선수들'). 이미지에 없는 내용(장소명, 매장명, 상황)을 절대 지어내지 말 것. 가능하면 키워드 '{$keyword}' 자연스럽게 포함하되, 실제 이미지와 어긋나면 넣지 말 것.\n\n";
        $user .= "## 출력 형식 (JSON만)\n";
